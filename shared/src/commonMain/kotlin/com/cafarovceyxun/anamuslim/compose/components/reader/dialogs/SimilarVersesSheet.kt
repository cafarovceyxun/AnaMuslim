...
import com.cafarovceyxun.anamuslim.resources.countSimilarVerses
import com.cafarovceyxun.anamuslim.resources.similarVerseRowMeta
                ...
                LazyColumn {
                    items(
                        ...
                        SimilarVerseItem(
                            ...
                            onOpenInReader = {
                                ReaderUiHooks.openVerse?.invoke(row.surahNo, row.verseNo)
                                onDismiss()
                            },
                        )
...
private fun Header(verse: VerseWithDetails, count: Int) {
    Column(
        ...
        Text(
            stringResource(Res.string.similarVerses),
            style = typography.titleMedium.merge(tightTextStyle),
        )

        Text(
            text = stringResource(
                Res.string.countSimilarVerses,
                stringResource(
                    Res.string.strLabelVerseWithChapNameAndNo,
    ...
    Column(
        ...
        Row(
            ...
            Column(
                ...
                Text(
                    text = stringResource(
                        Res.string.similarVerseRowMeta,
                        row.entity.coverage,
                        row.entity.score,
                    ),
...
private fun parseMatchWordIndices(json: String): Set<Int> {
    ...
    return try {
        ...
        for (pair in Json.decodeFromString<List<List<Int>>>(json)) {
            val from = pair[0]
            val to = pair[1]
