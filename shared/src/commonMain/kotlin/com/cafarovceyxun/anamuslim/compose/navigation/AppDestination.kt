...
sealed interface AppDestination {
    ...
    /**
     * One hadith, addressed by its id, opened **on top of** whatever is on screen — today the
     * homepage's hadith-of-the-day "read" button, which is where Android starts `ActivityHadith`
     * with a `hadith_id` extra.
     *
     * A pushed route rather than the tab root [HadithIndex], for the reason spelled out in
     * [SettingsDetail]: pushing a tab root corrupts the bar's saved stacks. The screen resolves the
     * id to its volume/book/chapter itself and drills in without a route per level, so the whole
     * hierarchy sits in this one entry and back leaves in one hop.
     */
    @Serializable
    data class HadithDetail(val hadithId: Long) : AppDestination
