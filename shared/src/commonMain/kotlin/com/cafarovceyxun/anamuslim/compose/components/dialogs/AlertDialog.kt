...
fun AlertDialog(
    ...
    val (maxDialogWidth, maxDialogHeight) = remember(windowInfo.containerSize, density) {
        with(density) {
            val width = minOf(610.dp, windowInfo.containerSize.width.toDp() * 0.9f)
            val height = minOf(560.dp, windowInfo.containerSize.height.toDp() * 0.9f)
            width to height
        }
    }
