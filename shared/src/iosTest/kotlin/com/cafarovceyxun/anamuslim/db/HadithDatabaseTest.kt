...
class HadithDatabaseTest {
    ...
    fun hierarchyRoundTripAndCascadingDelete() = runBlocking {
        ...
        // LIKE search on text_az. Empty arabicQuery switches the Arabic branch off — see
        // [arabicSearchIgnoresDiacriticsAndAlifVariants] for that half.
        assertEquals(1, dao.searchHadiths("niyyet", arabicQuery = "", limit = 10, offset = 0).size)
    ...
    /**
     * The Arabic half of [com.cafarovceyxun.anamuslim.db.dao.HadithDao.searchHadiths]: an
     * eighteen-deep `replace()` chain that strips harakat and folds the alif variants, so a reader
     * typing plain Arabic still matches vocalised text. Worth its own native test because the chain
     * leans on `char()` codepoint arguments — the kind of SQL that compiles anywhere and can still
     * behave differently on the bundled SQLite driver.
     */
    @Test
    fun arabicSearchIgnoresDiacriticsAndAlifVariants() = runBlocking {
        val db = db()
        val dao = db.hadithDao()

        dao.replaceAll(
            volumes = listOf(HadithVolumeEntity(slug = "bukhari", name = "Sahih Bukhari")),
            books = listOf(
                HadithBookEntity(slug = "b-iman", volume_slug = "bukhari", book_no = 1, name = "Iman"),
            ),
            chapters = listOf(
            ...
            hadiths = listOf(
                // Plain, already-normalised form.
                HadithEntity(id = 1L, chapter_slug = "c1", hadith_no = 1, text_ar = "نية", text_az = "Bir"),
                // Same word carrying harakat and a shadda.
                HadithEntity(id = 2L, chapter_slug = "c1", hadith_no = 2, text_ar = "نِيَّةٌ", text_az = "Iki"),
                // Alif with hamza below (U+0625), which folds to a plain alif (U+0627).
                HadithEntity(id = 3L, chapter_slug = "c1", hadith_no = 3, text_ar = "إيمان", text_az = "Uc"),
            ),
        )

        // `query` is deliberately unmatchable, so only the Arabic branch can produce hits.
        val noAzMatch = "zzz"

        // Both the plain and the vocalised spelling answer to the plain query.
        assertEquals(2, dao.searchHadiths(noAzMatch, arabicQuery = "نية", limit = 10, offset = 0).size)

        // The alif fold: typed with a plain alif, stored with hamza below.
        val alifHits = dao.searchHadiths(noAzMatch, arabicQuery = "ايمان", limit = 10, offset = 0)
        assertEquals(1, alifHits.size)
        assertEquals(3L, alifHits.single().id)

        // An empty arabicQuery must not turn into a match-everything LIKE '%%'.
        assertEquals(0, dao.searchHadiths(noAzMatch, arabicQuery = "", limit = 10, offset = 0).size)
