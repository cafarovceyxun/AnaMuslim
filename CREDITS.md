...
| `shared/src/commonMain/composeResources/files/atlas/uthmani/6x.zip` | Pre-shaped glyph atlas for the Uthmani mushaf (21 497 word forms, 747 glyphs) | Generated from the bundled KFGQPC font + this repo's own Qur'an text | ✅ **OK to bundle** — produced offline by `tools/atlas/` from `composeResources/font/uthmanic_hafs.ttf` and `assets/db/quranapp.db`; no third-party bundle is redistributed. The rendered glyphs remain KFGQPC's under the font licence summarised below, and are **not** GPLv3. |
